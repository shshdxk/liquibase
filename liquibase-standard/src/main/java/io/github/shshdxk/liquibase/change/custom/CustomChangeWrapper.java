...
import java.util.*;
import io.github.shshdxk.liquibase.util.OsgiUtil;
...
public class CustomChangeWrapper extends AbstractChange {
    ...
    /**
     * Specify a parameter on the CustomChange object to set before executing {@link Change#generateStatements(Database)}  or {@link #generateRollbackStatements(Database)} on it.
    ...
    /**
     * Call the {@link CustomChange#validate(Database)} method and return the result.
     */
    @Override
    ...
    /**
     * Finishes configuring the CustomChange based on the values passed to {@link #setParam(String, String)} then calls {@link CustomSqlChange#generateStatements(Database)}
     * or {@link CustomTaskChange#execute(Database)} depending on the CustomChange implementation.
     * <p></p>
    ...
    /**
     * Finishes configuring the CustomChange based on the values passed to {@link #setParam(String, String)} then calls {@link CustomSqlRollback#generateRollbackStatements(Database)}
     * or {@link CustomTaskRollback#rollback(Database)} depending on the CustomChange implementation.
     * <p></p>
     * If the CustomChange returns a null SqlStatement array, this method returns an empty array. If a CustomTaskChange is being used, this method will return an empty array.
